	...
	public List<GetAllCustomerAccountDetailsResponse> getCustomerAccountDetails() {
		...
		for(int i=0;i<tempUser.size();i++)
		{
			...
			//tempUser.get(i).getRole().getRoleType().equals("Customer")
				...
				{
					if(tempUser.get(i).getId()==tempAcc.get(j).getUserId() && tempUser.get(i).getRole().getRoleName().equals("ROLE_CUSTOMER"))
					{
						...
						tempCustomer1.setAccountStatusId(tempAcc.get(j).getAccountStatusId());
						tempCustomer.add(tempCustomer1);
					}
				}
